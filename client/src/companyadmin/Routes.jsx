...
function Routes() {
    ...
    const { all_routes, delete_route, routes, loading, error, setRoutes } = useContext(CompanyAdminContext);
    ...
    useEffect(() => {
        if (access_token && !hasFetched) {
            console.log("Fetching routes...");
            all_routes();
            setHasFetched(true);
        }
    }, [access_token, all_routes, hasFetched]);

    const handleDeleteRoute = async (id) => {
        console.log("Preparing to delete route with ID:", id);

        try {
            // Attempt to delete the route
            await delete_route(id);
            // If successful, update the state to remove the deleted route from the list
    ...
    return (
        <>
            <header>
                <div className="px-4 py-3 border border-indigo-200 rounded-xl mb-3 sm:px-6 bg-white">
                <h3 className="text-xl font-semibold text-gray-800">Registered Routes</h3>
                <p className="mt-1 text-sm text-gray-600">
                    You are viewing all registered Routes.
                </p>
                </div>
            </header>
            <div>
                <div className="flex flex-col">
                    <div className="overflow-x-auto pb-4">
                        <div className="min-w-full inline-block align-middle">
                            <div className="overflow-hidden border rounded-lg border-indigo-200">
                                        ...
                                        <tr className="bg-gray-50">
                                            <th>
                                                <div className="flex items-center py-5 px-5">
                                                    <input
                                                        type="checkbox"
                                                        className="w-5 h-5 appearance-none border border-gray-300 rounded-md mr-2 hover:border-dark-500 hover:bg-dark-100 checked:bg-no-repeat checked:bg-center checked:border-dark-500 checked:bg-dark-100"
                                                    />
                                                </div>
                                            </th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">Start Location</th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">End Location</th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">Distance</th>
                                            ...
                                            >
                                                <td className="p-5">
                                                    <div className="flex items-center">
                                                        <input
                                                            type="checkbox"
                                                            className="w-5 h-5 appearance-none border border-gray-300 rounded-md mr-2"
                                                        />
                                                    </div>
                                                </td>
                                                <td className="p-5 text-sm text-gray-900">{route.departure_county_name}</td>
                                                <td className="p-5 text-sm text-gray-900">{route.arrival_county_name}</td>
                                                <td className="p-5 text-sm text-gray-900">{route.distance}</td>
                                                <td className="flex p-5 items-center gap-1">
                                                    ...
                                                    <button onClick={() => handleDeleteRoute(route.id)} className="p-2 rounded-full bg-white hover:bg-red-600 flex">
                                                    <svg class="cursor-pointer" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    <button onClick={() => handleRouteClick(route)} class="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-black flex item-center">
