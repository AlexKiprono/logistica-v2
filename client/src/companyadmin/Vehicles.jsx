...
function Vehicles() {
    ...
    const { all_vehicles, delete_vehicle, vehicles, loading, error, setVehicles } = useContext(CompanyAdminContext);
    const [isModalOpen, setModalOpen] = useState(false);
    const [vehicleToDelete, setVehicleToDelete] = useState(null);
    const [hasFetched, setHasFetched] = useState(false); 

    useEffect(() => {
        if (auth_token && !hasFetched) {
            all_vehicles();  
            setHasFetched(true); 
        }
    }, [auth_token, all_vehicles, hasFetched]); 
    ...
    const confirmDelete = async () => {
        try {
            await delete_vehicle(vehicleToDelete);
        ...
        <>     
            <header>
                <div className="px-4 py-3 border border-indigo-100 rounded-xl mb-3 sm:px-6 bg-white">
                    <h3 className="text-xl font-semibold text-gray-800">Registered Vehicles</h3>
                    <p className="mt-1 text-sm text-gray-600">
                        You are viewing all registered Vehicles.
                    </p>
                </div>
            </header>
            <div>
                                        ...
                                        <tr className="bg-gray-50">
                                            <th>
                                                <div className="flex items-center py-5 px-5">
                                                    <input type="checkbox" className="w-5 h-5 appearance-none border border-gray-300 rounded-md mr-2 hover:border-indigo-500 hover:bg-indigo-100 checked:bg-no-repeat checked:bg-center checked:border-indigo-500 checked:bg-indigo-100"/>
                                                </div>
                                            </th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">License Plate</th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">Vehicle Type</th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">Capacity</th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">Status</th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">Enrol Date</th>
                                            <th className="p-5 text-left text-sm leading-6 font-semibold text-gray-900">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-300">
                                        {vehicles.map(vehicle => {
                                            const imageUrl = `/images/vehicles/${vehicle.image}`;
                                            return (
                                                <tr key={vehicle.id} className="bg-white transition-all duration-500 hover:bg-gray-50">
                                                    <td className="p-5">
                                                        <div className="flex items-center">
                                                            <input type="checkbox" className="w-5 h-5 appearance-none border border-gray-300 rounded-md mr-2"/>
                                                        </div>
                                                    </td>
                                                    <td className="p-5 text-sm text-gray-900">{vehicle.license_plate}</td>
                                                    <td className="p-5 text-sm text-gray-900">
                                                        <div className="flex items-center gap-3">
                                                            <img src={imageUrl} alt="Vehicle" className="w-10 h-10 object-cover"/>
                                                            <p className="font-normal text-sm">{vehicle.vehicle_type}</p>
                                                        </div>
                                                    </td>
                                                    <td className="p-5 text-sm text-gray-900">{vehicle.capacity}</td>
                                                    <td className="p-5 text-sm text-gray-900">
                                                        <div className={`py-1.5 px-2.5 rounded-full flex justify-center items-center gap-1 ${vehicle.status === 'in use' ? 'bg-emerald-50' : 'bg-yellow-100'}`}>
                                                            <svg width="5" height="6" viewBox="0 0 5 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                <circle cx="2.5" cy="3" r="2.5" fill={vehicle.status === 'in use' ? '#059669' : '#F59E0B'} />
                                                            </svg>
                                                            <span className={`font-medium text-xs ${vehicle.status === 'in use' ? 'text-emerald-600' : 'text-yellow-600'}`}>
                                                                {vehicle.status}
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td className="p-5 text-sm text-gray-900">{vehicle.created_at}</td>
                                                ...
                                                <button class="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
                                                    ...
                                                    <button onClick={() => handleDeleteVehicle(vehicle.id)} className="p-2 rounded-full bg-white hover:bg-red-600 hover:text-white flex">
                                                    <svg class="cursor-pointer" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-75 z-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-96">
                        ...
                        <p className="mt-2 text-sm text-gray-600">Are you sure you want to delete this vehicle?</p>
                        <div className="flex items-center gap-4 mt-4">
                            <button onClick={cancelDelete} className="bg-gray-500 text-white px-4 py-2 rounded-md">
                                Cancel
                            </button>
