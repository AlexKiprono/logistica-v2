...
function Driver() {
    ...
    const { all_drivers,all_vehicles, vehicles, delete_driver, drivers, loading,assignVehicleToDriver, handleRemoveVehicle, error, setDrivers } = useContext(CompanyAdminContext);
    const [isModalOpen, setModalOpen] = useState(false);
    const [driverToDelete, setDriverToDelete] = useState(null);
    const [isAssignVehicleModalOpen, setAssignVehicleModalOpen] = useState(false);
    ...
    const confirmAssignVehicle = async () => {
        if (selectedDriver && selectedVehicle) {
            await assignVehicleToDriver(selectedDriver.id, selectedVehicle.id);
    ...
    const confirmDelete = async () => {
        await delete_driver(driverToDelete);
        const updatedDrivers = drivers.filter(driver => driver.id !== driverToDelete);
    ...
    <header>
        <div className="px-4 py-3 border border-indigo-100 rounded-xl mb-3 sm:px-6 bg-white">
          <h3 className="text-xl font-semibold text-gray-800">Registered Drivers</h3>
          <p className="mt-1 text-sm text-gray-600">
            You are viewing all registered Drivers.
          </p>
        </div>
      </header>
      ...
      <div className="flex flex-col">
                            ...
                            <thead>
                                <tr className="bg-gray-50">
                                    <th className="p-5 text-left">
                                        <input type="checkbox" className="w-5 h-5 border rounded-md" />
                                    </th>
                                    <th className="p-5 text-left text-sm font-semibold text-gray-900">License</th>
                                    <th className="p-5 text-left text-sm font-semibold text-gray-900">Full Name & Email</th>
                                    <th className="p-5 text-left text-sm font-semibold text-gray-900">Status</th>
                                    <th className="p-5 text-left text-sm font-semibold text-gray-900">Vehicle</th>
                                    <th className="p-5 text-left text-sm font-semibold text-gray-900">License Expiry</th>
                                    <th className="p-5 text-left text-sm font-semibold text-gray-900">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-300">
                                {drivers.map((driver) => {
                                    const licenseExpiry = new Date(driver.license_expiry).toLocaleDateString(); // Format license expiry date

                                    return (
                                        <tr key={driver.id} className="bg-white hover:bg-gray-50">
                                            <td className="p-5">
                                                <input type="checkbox" className="w-5 h-5 border rounded-md" />
                                            </td>
                                            <td className="p-5">
                                                <p className="font-normal text-sm text-gray-900">{driver.license_number}</p>
                                                <p className="font-normal text-xs text-gray-400">Expiry: {licenseExpiry}</p>
                                            </td>
                                            <td className="p-5">
                                                <div className="flex items-center gap-3">
                                                    <img src="https://pagedone.io/asset/uploads/1697536419.png" alt="Driver Avatar" className="w-10 h-10 rounded-full" />
                                                    <div>
                                                        <p className="font-normal text-sm text-gray-900">{driver.first_name} {driver.last_name}</p>
                                                        <p className="font-normal text-xs text-gray-400">{driver.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-5 text-sm text-gray-900">
                                                <div className={`py-1.5 px-2.5 rounded-full flex justify-center items-center gap-1 ${driver.assigned_vehicle ? 'bg-emerald-50' : 'bg-red-50'}`}>

                                                    <svg width="5" height="6" viewBox="0 0 5 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <circle cx="2.5" cy="3" r="2.5" fill={driver.assigned_vehicle ? '#059669' : '#EF4444'} />
                                                    </svg>
                                                    <span className={`font-medium text-xs ${driver.assigned_vehicle ? 'text-emerald-600' : 'text-red-600'}`}>
                                                        {driver.assigned_vehicle ? 'Active' : 'Inactive'}
                                                    </span>
                                                </div>
                                            </td>
                                                ...
                                                <p className="font-normal text-sm text-gray-900">
                                                    {driver.assigned_vehicle ? (
                                                        <>
                                                            {driver.assigned_vehicle}  
                                                            <button
                                                                onClick={() => handleRemoveVehicle(driver.id)}  
                                                                className="ml-2 text-red-500 hover:text-red-700"
                                                            >
                                                                <svg class="w-6 h-6 text-red-600 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 24 24">
                                            ...
                                            <td className="p-5 text-sm text-gray-900">{licenseExpiry}</td>
                                            <td className="flex p-5 items-center gap-1">
                                                <button class="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
                                                    ...
                                                    <button onClick={() => handleDeleteDriver(driver.id)} className="p-2 rounded-full bg-white hover:bg-red-600 hover:text-white flex">
                                                    <svg class="cursor-pointer" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    <button onClick={() => handleAssignVehicle(driver)} class="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-black flex item-center">
                                                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path class="stroke-black group-hover:stroke-white" d="M10.0161 14.9897V15.0397M10.0161 9.97598V10.026M10.0161 4.96231V5.01231" stroke="black" strokeWidth="2.5" strokeLinecap="round"></path>
                                                        </svg>
                                                    </button>
                                                </td>
                                        </tr>
            ...
            {isAssignVehicleModalOpen && (
                <AssignVehicleModal
                    isAssignVehicleModalOpen={isAssignVehicleModalOpen}
                    closeModal={setAssignVehicleModalOpen}
                    vehicles={vehicles}
                    selectedVehicle={selectedVehicle}
                    setSelectedVehicle={setSelectedVehicle}
                    confirmAssignVehicle={confirmAssignVehicle}
                    cancelAssignVehicle={cancelAssignVehicle}
